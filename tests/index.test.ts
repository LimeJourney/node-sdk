...
import { APIUserAbortError } from 'limejourney';
import { Headers } from 'limejourney/core';
...
describe('instantiate client', () => {
  ...
  describe('defaultQuery', () => {
    ...
    test('overriding with `undefined`', () => {
      const client = new Limejourney({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Limejourney({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['LIMEJOURNEY_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Limejourney({ baseURL: 'https://example.com' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Limejourney({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Limejourney({});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Limejourney({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Limejourney({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Limejourney({ fetch: testFetch });
